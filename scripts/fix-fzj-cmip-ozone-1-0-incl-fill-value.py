...
This time, don't muck up the fill value.
As FZJ-CMIP-ozone-1-0 is already published,
also bump to FZJ-CMIP-ozone-1-1.
"""
...
def rewrite_clim_qbo_files(  # noqa: PLR0913
    clim_qbo_files: list[Path],
    out_dir: Path,
    correct_contact: str,
    patch_source_id: str,
    patch_source_version: str,
    verbose: bool = True,
) -> list[Path]:
    """
    ...
    for f in clim_qbo_files:
        ...
        ds_fixed.setncattr("source_id", patch_source_id)
        ds_fixed.setncattr("source_version", patch_source_version)
...
def rewrite_clim_files(  # noqa: PLR0913
...
def rewrite_historical_files(  # noqa: PLR0913
...
def main() -> None:
    ...
    CV_SOURCE = "gh:ozone-patch"
    SOURCE_ID_OUT = "FZJ-CMIP-ozone-1-1"
    source_version_out = SOURCE_ID_OUT.split("-ozone-")[-1].replace("-", ".")
    ...
    rewritten_clim_qbo_files = rewrite_clim_qbo_files(
        clim_qbo_files,
        out_dir=TMP_DIR,
        correct_contact=cvs.source_id_entries[SOURCE_ID_OUT].values.contact,
        patch_source_id=SOURCE_ID_OUT,
        patch_source_version=source_version_out,
    )
